...
#define msg(s) cerr << "[RainSim] " << s << endl;

const string SPHERE = "sphere";
const string PLANE = "plane";
const string MESH = "mesh";

const unordered_set<string> VALID_KEYS = {SPHERE, PLANE, MESH};

RainSimulator *app = nullptr;
...
void createGLContexts() {
  ...
  window = glfwCreateWindow(800, 800, "Rain Simulator", nullptr, nullptr);
  if (window == nullptr) {
  ...
  for (json::iterator it = j.begin(); it != j.end(); ++it) {
    ...
    if (key == SPHERE) {
      ...
      objects->push_back(p);
        ...
        // TODO: perhaps implement mesh loading here
    }
  }
...
int main(int argc, char **argv) {
  ...
  // Initialize the RainSimulator object
  app = new RainSimulator(project_root, screen);
