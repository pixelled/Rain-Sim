...
void main() {
    ...
    const float shadow_radius = 1;
    float shadow_factor = 1 - (max(shadow_radius - length(vec3(v_position) - shadow_center), 0) / shadow_radius);

    vec3 diffuse = I * vec3(color) * shadow_factor * max(0.f, dot(nd, l)) * (1 - wetness - base_wetness) * diffuse_brightness;
